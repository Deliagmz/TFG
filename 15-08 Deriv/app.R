...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    tabPanel("Derivación",
             ...
             tabsetPanel(
               ...
               tabPanel(
                 ...
                 mainPanel(navlistPanel(
                   tabPanel("Función complicada ",mainPanel(
                     
                     fluidRow(
                       splitLayout(cellWidths = 325,h4(""),
                                   h4(" $$f(x)=x^{(e-x^2)}e^{(-x^2)}(-2xlnx+\\frac{1}{x})$$  $$x=1,3$$ "), h4("")),
                       br(),
                       splitLayout(cellWidths = 500,
                             fluidPage( 
                               verticalLayout(
                                    radioButtons(
                                   "metodosDer", "Selecciona una fórmula:",
                                   ...
                                   fluidPage( 
                                     verticalLayout(sliderInput("h", "h:", min = 0.01,max =0.2,value=0.05))),
                     ...
                     fluidRow(
                       splitLayout(
                         ...
                         fluidPage( 
                           verticalLayout(
                                uiOutput("textDerC"),
                                actionButton("botonDerC", "",icon = icon("refresh"), style = "color: #fff; background-color: #f2540c; border-color: #f2540c")
                           )),
                         plotOutput("graficaDerC",height = 600,width = 500),
                   ...
                   tabPanel("Función sencilla",mainPanel(
    ...
    navbarMenu(
      ...
      tabPanel(
        "Fórmula abierta de Newton-Côes",
          ...
          tabPanel("Teoría", uiOutput("teoríaIAN"),
                   mainPanel(fluidRow(
                     column(
                       ...
                       br(),
                       br(),
                       h4("Fórmula abierta de Newton-Côes "),
                       br(),
                       p("La fórmula abierta de Newton-Côes de n+1 puntos utiliza los nodos: "),
                       p(
...
server <- function(input, output) { 
 ...
 cont<-0
  ...
  observeEvent(input$botonDerC,{
    ...
    output$cent <- renderText( paste( withMathJax("$$f'(1.3) = \\frac{f(1.3+",h,")-f(1.3-",h,")}{2*",h,"} =",derC,"$$")))
    output$izq <- renderText( paste( withMathJax("$$f'(1.3) = \\frac{f(1.3)-f(1.3-",h,")}{2*",h,"} =",derI,"$$")))
    output$der <- renderText( paste( withMathJax("$$f'(1.3) = \\frac{f(1.3+",h,")-f(1.3)}{2*",h,"} =",derD,"$$")))
    
    if(input$metodosDer== "Centrada" ){
      output$textDerC <- renderText( paste( withMathJax("$$f'(1.3) = \\frac{f(1.3+",h,")-f(1.3-",h,")}{2*",h,"} =",derC,"$$")))
      
    }else if(input$metodosDer== "Descentrada izquierda" ){
      output$textDerC <- renderText( paste( withMathJax("$$f'(1.3) = \\frac{f(1.3)-f(1.3-",h,")}{2*",h,"} =",derI,"$$")))
      
    } else {
      output$textDerC <-renderText( paste( withMathJax("$$f'(1.3) = \\frac{f(1.3+",h,")-f(1.3)}{2*",h,"} =",derD,"$$")))
      
    }
   
    
    xDerC <- seq(1, 1.5, 0.05)
    xDerCdf <- as.data.frame(xDerC)
    slopeDerC <- function(x) {
      xincrementDerC <- 0.2
      yincrementDerC <-
        ((x+0.1)^(exp(1)-(x+0.1)^2)*exp((-(x+0.1))^2)*(-2*(x+0.1)*ln(x+0.1)+(1/(x+0.1))))-((x-0.1)^(exp(1)-(x-0.1)^2)*exp((-(x-0.1))^2)*(-2*(x-0.1)*ln(x-0.1)+(1/(x-0.1))))
      slopeDerc <- yincrementDerC / xincrementDerC
      return(slopeDerc)
    }
    
    interceptDerC <- function(x) {
      interceptDerc <- (x)^(exp(1)-(x)^2)*exp((-(x))^2)*(-2*(x)*ln(x)+(1/(x))) - slopeDerC(x) * x
      yvaluesDerC <- slopeDerC(x) * xDerC + interceptDerc
      xDerCdf <- as.data.frame(cbind(xDerCdf, yvaluesDerC))
      return(interceptDerc)
    } 
    
    centG <- ggplot(xDerCdf, aes(x = xDerC, y =  xDerC^(exp(1)-xDerC^2)*exp((-xDerC)^2)*(-2*xDerC*ln(xDerC)+(1/xDerC))))+
      ...
      geom_line(data = xDerCdf) +
      geom_line(color = "blue", size = 1)
    
    izqG <- ggplot(xDerCdf, aes(x = xDerC, y =  xDerC^(exp(1)-xDerC^2)*exp((-xDerC)^2)*(-2*xDerC*ln(xDerC)+(1/xDerC))))+
      geom_line(aes(x = 1.3,colour = "a"),color="#f2540c", size = 1,linetype = 5) +
      geom_line(aes(x = 1.3-h,colour = "a-h"),color="#f2540c", size = 1,linetype = 5) +
      geom_abline(
        slope = slopeDerC(1.3),
    ...
    derG <- ggplot(xDerCdf, aes(x = xDerC, y =  xDerC^(exp(1)-xDerC^2)*exp((-xDerC)^2)*(-2*xDerC*ln(xDerC)+(1/xDerC))))+
      geom_line(aes(x = 1.3,colour = "a"),color="#f2540c", size = 1,linetype = 5) +
      geom_line(aes(x = 1.3+h,colour = "a+h"),color="#f2540c", size = 1,linetype = 5)+
      geom_abline(
        slope = slopeDerC(1.3),
    ...
    if(input$metodosDer== "Centrada" ){
      output$graficaDerC <- renderPlot(centG)
    }else if(input$metodosDer== "Descentrada izquierda" ){
      output$graficaDerC <- renderPlot(izqG)
    } else {
      output$graficaDerC <- renderPlot(derG)
    }
