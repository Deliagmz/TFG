          ...
          mainPanel(navlistPanel(
            tabPanel("Ecuación exponencial",  mainPanel(
              fluidRow(
                ...
                h4("Ecuación exponencial  $$f(x)=e^{(3x)}+x^2-4$$ $$f'(x)=3e^{(3x)}+2x$$"), 
                fluidPage( 
...
server <- function(input, output) { 
 ...
 xDT <- as.data.frame(x)
  ...
  bipartPC <- function(i, n,abPC) {
    if (i <= n) {
      output$ptoscorte <- renderTable(dfPC[1:i, ])
      output$graficaPC <-
        renderPlot(
          gBipartPC +
  ...
  observeEvent(input$botonPC, bipartPC(input$botonPC, 5, abPC))
  ...
  newtonExp <- function(i, n) {
    if (i <= n) {
      output$expN <- renderTable(dfNII[1:i,])
  ...
  slope <- function(x) {
    xincrement <- 0.2
    yincrement <-
      exp( (x + 0.1)) + (x + 0.1) ^ 2 - 4 - exp( (x - 0.1)) + (x - 0.1) ^
      2 - 4
